...
class GOOSEDataExplorer:
    ...
    def plot_event_occurrence_timeline(self, df: pd.DataFrame, logger: WandbLogger = None):
        ...
        # Print detailed analysis for sample gocbRefs
        print("\nDetailed Event Analysis for Sample gocbRefs:")
        print("-" * 70)
        
        for attack_type in attack_types[:2]:  # Analyze first 2 attack types in detail
            subset = df[df['attack_type'] == attack_type]
            
            # Get most common gocbRef
            if len(subset) > 0 and 'gocbRef' in subset.columns:
                common_gocbref = subset['gocbRef'].value_counts().index[0]
                gocb_data = subset[subset['gocbRef'] == common_gocbref].sort_values('Time')
                
                if len(gocb_data) > 1 and 'stNum' in gocb_data.columns:
                    stnum_changes = (gocb_data['stNum'].diff() != 0).sum()
                    attacks_during_events = ((gocb_data['stNum'].diff() != 0) & 
                                            (gocb_data['attack'] == 1)).sum()
                    
                    print(f"\n{attack_type.upper()} - gocbRef: {common_gocbref[:40]}")
                    print(f"  Total messages: {len(gocb_data)}")
                    print(f"  stNum changes (events): {stnum_changes}")
                    print(f"  Attacks during events: {attacks_during_events}")
                    
                    if stnum_changes > 0:
                        print(f"  Attack rate during events: {attacks_during_events/stnum_changes:.2%}")
...
def run_task1(config: Dict[str, Any], logger: WandbLogger = None) -> Dict:
    ...
    explorer.plot_attack_distribution(train_data, logger)
    explorer.plot_temporal_patterns(train_data, logger)
    explorer.plot_feature_distributions(train_data, logger)
    explorer.plot_correlation_analysis(train_data, logger)
    explorer.plot_dimensionality_reduction(train_data, logger)
    
    # Phase 2: Deep dive analyses
    print("\nPhase 2: Deep dive analyses...")
    explorer.analyze_temporal_and_categorical_patterns(train_data, logger)
    explorer.plot_event_occurrence_timeline(train_data, logger)
    explorer.analyze_goose_rules(train_data, logger)
