...
def create_windows(X, y, window_size=10):
    """
    Converts (N, D) tabular data into (N - window_size, window_size, D) sequences.
    """
    X_seq, y_seq = [], []
    for i in range(len(X) - window_size):
        X_seq.append(X[i : i + window_size])
        # We target the label of the LAST packet in the window
...
class TemporalLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # Use the hidden state of the final time step
        last_time_step = out[:, -1, :]
        return self.fc(last_time_step)
    
def run_lstm(X_train, y_train, window_size=10, epochs=50, lr=1e-3, random_state=42):
    torch.manual_seed(random_state)
    
    # 1. Prepare sequential data
    X_win, y_win = create_windows(X_train, y_train, window_size)
    
    X_tensor = torch.tensor(X_win, dtype=torch.float32)
    y_tensor = torch.tensor(y_win, dtype=torch.float32).view(-1, 1)
...
def predict(model, X_test, window_size=10, threshold=0.5, batch_size=512):
    model.eval()
    
    # Create windows (dummy y as we only need X)
    X_win, _ = create_windows(X_test, np.zeros(len(X_test)), window_size)
    X_tensor = torch.tensor(X_win, dtype=torch.float32)
    ...
    scores = np.concatenate(all_probs, axis=0).flatten()
    y_pred = (scores >= threshold).astype(int)

    # Note: Predictions will have (len(X_test) - window_size) samples
