...
def run_nn(X_train, y_train,
    ...
    # Simple feedforward network: input -> h1 -> h2 -> output
    class SimpleNN(nn.Module):
        def __init__(self, input_dim, hidden1, hidden2):
            ...
            self.net = nn.Sequential(
                ...
                nn.ReLU(),
                nn.Linear(hidden2, 1)  # raw logit for binary class
            )
        def forward(self, x):
            return self.net(x)
    
    model = SimpleNN(input_dim, hidden_size1, hidden_size2)
    
    # Tensors (assume X_train, y_train are numpy arrays or similar)
...
def predict(model, X, device='cpu', threshold=0.5, batch_size=1024):
    """
    Returns:
      scores: numpy array, continuous anomaly score in [0,1] (sigmoid probability for class 1)
      y_pred: numpy array, binary labels (0 normal, 1 anomaly) using `threshold`
    """
    model.eval()
    all_probs = []

    # batch inference to avoid OOM on large X
    n = X.shape[0]
