...
def transform_to_1d_cnn(X):
    """
    Reshapes tabular data (N, D) to (N, 1, D) for Conv1d.
    """
...
class Tabular1DCNN(nn.Module):
    def __init__(self, input_dim):
        ...
        self.features = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool1d(2),
            nn.Dropout(0.2),  # Prevents overfitting on specific protocol fields
        ...
        with torch.no_grad():
            ...
            self.flat_dim = self.features(dummy).view(1, -1).size(1)
...
def run_cnn_1d(X_train, y_train, lr=1e-3, num_epochs=50, random_state=42):
    """
    Fits a 1D-CNN on the provided training data.
    """
    torch.manual_seed(random_state)
    
    # Transform data
    X_tensor = transform_to_1d_cnn(X_train)
    y_tensor = torch.tensor(y_train, dtype=torch.float32).view(-1, 1)
    
    input_dim = X_train.shape[1]
    model = Tabular1DCNN(input_dim)
...
def predict(model, X_test, threshold=0.5, batch_size=512):
    """
    Generates binary predictions and continuous anomaly scores (probabilities).
    """
    model.eval()
    X_tensor = transform_to_1d_cnn(X_test)
