# Gaussian Mixture Model (GMM, k=1) for Anomaly Detection (PyOD)
import os
import sys
import numpy as np
import pandas as pd
...
LABEL_COL   = 'attack'
ALGO_NAME   = 'gmm'  # used for plot-data folder
...
# -------------------------------------------------------------
# model selection / training (k=1, choose covariance_type)
# -------------------------------------------------------------
def run_gmm(X_train, X_val, y_val, contamination=0.1, random_state=None):
...
def predict(X, model):
    ...
    return y_pred.astype(int), scores


#-----------------------NEW CODE-------------------------------
# def predict(X, model):
#     # 1. Get raw predictions: -1 for anomaly, 1 for inlier
#     raw_pred = model.predict(X)
    
#     # 2. Map to your labels: 1 for Attack (anomaly), 0 for Normal (inlier)
#     y_pred = np.where(raw_pred == -1, 1, 0)
    
#     # 3. Correct Anomaly Scores for PR-AUC/ROC-AUC:
#     # decision_function = log-likelihood (high for normal, low for attack).
#     # We negate it so high scores = high probability of attack.
...
def main(train_input_path=None,
    ...
    # ---------- Contamination estimate (bounded) ----------
    contamination = n_val_attack / max(1, (n_val_attack + n_val_normal))
    contamination = max(0.001, min(0.49, contamination))

    # ---------- Hierarchy info for per-run CSVs ----------
...
if __name__ == "__main__":
    ...
    # print("GMM (1-component) complete.")
