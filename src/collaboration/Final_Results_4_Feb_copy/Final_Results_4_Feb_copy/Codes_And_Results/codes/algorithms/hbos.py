...
def fit_hbos(
    ...
    """
    Fit Histogram-Based Outlier Score (HBOS) on normal-only data.
    ...
    percentile : float
        Percentile for thresholding (learned from train)
    ...
    for j in range(n_features):
        ...
        hist = np.maximum(hist, eps)  # avoid zero density
        histograms.append(hist)
        bin_edges.append(edges)

    # Compute training scores to set threshold
    train_scores = _hbos_score(X_train, histograms, bin_edges, eps)
...
def _hbos_score(X, histograms, bin_edges, eps):
    """
    Internal HBOS score computation.
    ...
    for j in range(n_features):
        ...
        # Find histogram bin indices
        bin_idx = np.searchsorted(edges, X[:, j], side="right") - 1
        bin_idx = np.clip(bin_idx, 0, len(hist) - 1)

        # Density lookup
        densities = hist[bin_idx]

        # Accumulate negative log-density
        scores += -np.log(densities + eps)
...
def predict(X_test, model):
    """
    Predict anomalies using trained HBOS model.
