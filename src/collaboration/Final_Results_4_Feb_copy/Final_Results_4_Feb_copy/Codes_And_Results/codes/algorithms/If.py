...
LABEL_COL   = 'attack'
ALGO_NAME   = 'if'
IF_DATA_DIR = 'c:/Users/Rayaan_Ghosh/Desktop/OSS/cp219_project-2/src/collaboration/Final_Results_4_Feb_copy/Final_Results_4_Feb_copy/OCT7Files/unsupervised_codes_Shailja/isolation_forest_data'
...
def predict(model, X):
    """
    Returns:
      scores: continuous anomaly scores (higher => more anomalous)  <-- use for PR/ROC
      y_pred: binary labels (0 normal, 1 anomaly)                    <-- thresholded/predicted labels
    """
    ...
    # binary labels: model.predict -> -1 anomaly, +1 normal
    iso_labels = model.predict(X)
    y_pred = np.where(iso_labels == -1, 1, 0).astype(int)
...
def main(train_input_path=None,
    ...
    for i in range(1, num_runs + 1):
        ...
        try:
            ...
            pr_auc = average_precision_score(y_test, scores)   # PR-AUC
            roc_auc = roc_auc_score(y_test, scores)            # ROC-AUC
