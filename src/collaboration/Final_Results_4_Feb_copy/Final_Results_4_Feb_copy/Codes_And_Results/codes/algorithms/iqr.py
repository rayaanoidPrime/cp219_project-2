...
#   Core helpers
# =========================
def fit_iqr(X_train, k=1.5, eps=1e-8):
    """
    Fit an IQR-based anomaly detector on normal-only data.
    ...
    -------
    model : dict
        Stored statistics required for prediction
    """
...
def predict(X, model):
    """
    Predict anomalies using a fitted IQR model.

    Parameters
    ----------
    X : np.ndarray (M, d)
        Test data (normal + attack)
    model : dict
        Output of fit_iqr()
    ...
    -------
    y_pred : np.ndarray (M,)
        Binary anomaly predictions (1 = anomaly)
    scores : np.ndarray (M,)
    ...
    # Amount by which each feature violates the IQR bounds
    lower_violation = np.maximum(0.0, lower - X)
    upper_violation = np.maximum(0.0, X - upper)

    # Normalize violations by IQR (scale-free)
    violations = (lower_violation + upper_violation) / IQR

    # Continuous anomaly score:
    # max normalized violation across features
    scores = np.max(violations, axis=1)

    # Binary decision:
    # anomaly if any feature violates bounds
...
def main(train_input_path=None,
    ...
    # Counts
    n_train_attack = uh.count_stat(y_train).get(1, 0)
    n_train_normal = uh.count_stat(y_train).get(0, 0)
    n_val_attack   = uh.count_stat(y_val).get(1, 0)
    n_val_normal   = uh.count_stat(y_val).get(0, 0)
    n_test_attack  = uh.count_stat(y_test).get(1, 0)
    n_test_normal  = uh.count_stat(y_test).get(0, 0)

    all_runs_results = {}
    N_LOOPS = 3
    num_runs = 1

    ps_dir = os.path.join(output_dir, "per_sample")
    ...
    for i in range(1, num_runs + 1):
        ...
        try:
            ...
            # ---- TEST (metrics + per-sample) ----
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):        
                    y_test_pred, l2_z  = predict(X_test, model)
            ...
            pr_auc = average_precision_score(y_test, l2_z)
            roc_auc  = roc_auc_score(y_test, l2_z)
