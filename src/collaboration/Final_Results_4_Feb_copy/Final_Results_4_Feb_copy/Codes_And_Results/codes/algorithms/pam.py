...
# =============================================================
# Core Runner for KMedoids (Distance-based)
# =============================================================
def run_pam(X_train, metric='euclidean', percentile=95, random_state=42):
    rng = np.random.default_rng(random_state)
    # 1. Fit PAM with k=1 on Normal-only data
    # This finds the single 'Medoid' that is most central to legitimate traffic
    seed= rng.integers(0, 100)
    model = KMedoids(n_clusters=1, metric=metric, method='pam', random_state=seed, max_iter=0)
    model.fit(X_train)

    # 2. Extract the Golden Medoid
    normal_medoid = model.cluster_centers_

    # 3. Calculate Training Distances to set a threshold
    train_distances = pairwise_distances(X_train, normal_medoid, metric=metric).flatten()
    threshold = np.percentile(train_distances, percentile)

    return normal_medoid, threshold

def predict(X_test, normal_medoid, threshold, metric='euclidean'):
    # 4. Measure distance of unseen data to the normal medoid
    scores = pairwise_distances(X_test, normal_medoid, metric=metric).flatten()
    
    # 5. Classify based on the learned threshold
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):
                   normal_medoid, threshold = run_pam(
                       X_train=X_train, random_state=i*10
                        )
            ...
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    y_test_pred, scores= predict(X_test,normal_medoid, threshold)
