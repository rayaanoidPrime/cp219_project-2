...
from sklearn.metrics import classification_report, confusion_matrix, f1_score, pairwise_distances
from scipy.optimize import linear_sum_assignment
...
def run_ward(X, y_true, n_clusters=2):
    # 1. Fit Ward
    model = AgglomerativeClustering(n_clusters=n_clusters, linkage='ward')
    cluster_labels = model.fit_predict(X)

    # 2. Hungarian Mapping
    contingency = np.zeros((n_clusters, 2))
    for c in range(n_clusters):
        for l in range(2):
            contingency[c, l] = np.sum((cluster_labels == c) & (y_true == l))
    
    # Minimize negative agreement = Maximize agreement
    row_ind, col_ind = linear_sum_assignment(-contingency)
    cluster_to_label = {cluster: label for cluster, label in zip(row_ind, col_ind)}
    
    # 3. Identify the 'Normal' Cluster index
    # Find which cluster ID was mapped to label 0
    normal_cluster_idx = [c for c, l in cluster_to_label.items() if l == 0][0]

    # 4. Calculate Centroids in raw feature space
    centroids = []
    for c in range(n_clusters):
        centroids.append(X[cluster_labels == c].mean(axis=0))
    centroids = np.array(centroids)
    normal_centroid = centroids[normal_cluster_idx].reshape(1, -1)

    # 5. Continuous Anomaly Score: Distance to Normal Centroid
    # Higher distance = More likely to be an attack
    scores = pairwise_distances(X, normal_centroid).flatten()

    # Hard predictions based on the Hungarian mapping
    ...
    for i in range(1, num_runs + 1):
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    y_test_pred, scores= run_ward(X_test, y_test)
