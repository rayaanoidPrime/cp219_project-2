...
class VAE(Model):
    ...
    def compute_loss(self, x):
        ...
        rec_loss = tf.reduce_mean(tf.reduce_sum(tf.square(x - reconstruction), axis=1))
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            total_loss, rec_loss, kl_loss = self.compute_loss(x)
...
def build_vae(input_dim, latent_dim=10):
    ...
    h_dec = Dense(64, activation="relu")(latent_inputs)
    outputs = Dense(input_dim, activation=None)(h_dec)
...
# Train Function
# =============================================================
def run_vae(X_tr, y_tr, random_state):
    """
    VAE training using ONLY normal training data (no validation set).
    One run = one random initialization.
    """

    # =====================================================
    # 1. SET ALL RANDOM SEEDS (ONCE PER RUN)
    # =====================================================
    ...
    # 2. BUILD MODEL
    # =====================================================
    vae, _, _ = build_vae(X_tr.shape[1])

    # =====================================================
    # 3. TRAIN (NO VALIDATION)
    # =====================================================
    early_stop = EarlyStopping(
        monitor="loss",          # <-- TRAIN LOSS ONLY
        patience=10,
    ...
    # =====================================================
    # 4. COMPUTE TRAINING RECONSTRUCTION ERROR
    # =====================================================
    train_recon = vae.predict(X_tr, verbose=0)
    train_mse = np.mean(np.square(X_tr - train_recon), axis=1)

    # Robust threshold (same as your other baselines)
    threshold = np.mean(train_mse) + 3.0 * np.std(train_mse)
...
# =============================================================
# Main driver (unchanged interface)
# =============================================================
        ...
        try:
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):
                        model, threshold= run_vae(X_train, y_train, random_state=i*10)
