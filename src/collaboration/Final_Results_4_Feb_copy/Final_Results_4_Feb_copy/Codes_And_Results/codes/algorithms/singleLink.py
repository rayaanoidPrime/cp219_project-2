...
# =============================================================
# Core Logic : Single Linkage
# =============================================================
def fit_single_linkage(X, depth=3, percentile=95):
    """
    Transductive Single Linkage anomaly detection using Scipy.
    
    Parameters:
    - X: The test data (Normal + Attack)
    - depth: Number of levels in the hierarchy to consider for inconsistency
    """
    N = X.shape[0]

    # 1. Generate the Single Linkage dendrogram
    # Result Z is: [idx1, idx2, distance, sample_count]
    Z = linkage(X, method='single', metric='euclidean')

    # 2. Calculate the Inconsistency Coefficient
    # This compares a merge height to the average height of its ancestors.
    # High inconsistency = the point is being forced into a cluster it doesn't fit.
    stats = inconsistent(Z, d=depth)
    
    # We map the inconsistency of the merges back to the original points.
    # As a simpler proxy for 'Single Linkage' anomaly detection:
    # Use the height of the LAST merge for each point (similar to your MST logic).
    scores = np.zeros(N)
    cluster_members = {i: [i] for i in range(N)}

    for k in range(Z.shape[0]):
        idx1, idx2, dist, _ = Z[k]
        idx1, idx2 = int(idx1), int(idx2)
        
        # Combine clusters
        members = cluster_members.pop(idx1) + cluster_members.pop(idx2)
        cluster_members[N + k] = members
        
        # Every time a point is merged, update its score to the current distance
        for p in members:
            scores[p] = dist
...
def main(train_input_path=None,
    ...
    for i in range(1, num_runs + 1):
        ...
        try:
            # if skip:
            #     raise Exception("Skipping this dataset as per user request.")
            # ---------------- Fit SingleLinkage ----------------
            with ru.ResourceProfiler() as profiler_train:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    _,_ = fit_single_linkage(X_test)
            avg_train_wall_ns = profiler_train.wall_nanoseconds / N_LOOPS
            avg_time_pkt_tr = avg_train_wall_ns / len(y_test) if len(y_test) else None
            ...
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    y_test_pred, scores = fit_single_linkage(X_test)
