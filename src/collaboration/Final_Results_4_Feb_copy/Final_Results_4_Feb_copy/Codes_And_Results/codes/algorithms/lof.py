...
LABEL_COL     = 'attack'
ALGO_NAME     = 'lof'
LOF_DATA_DIR  = 'c:/Users/Rayaan_Ghosh/Desktop/OSS/cp219_project-2/src/collaboration/Final_Results_4_Feb_copy/Final_Results_4_Feb_copy/OCT7Files/unsupervised_codes_Shailja/lof_data'
...
# Default LOF hyperparams (kept out of main signature for uniformity)
N_NEIGHBORS   = 20
CONTAMINATION = 0.05
...
# ==============================================================================
# Core primitives (resource-capturing, unchanged LOF logic)
# ==============================================================================
def run_lof(X_train,
            n_neighbors=N_NEIGHBORS,
            contamination=CONTAMINATION):
    """
    Fit LOF (novelty=True) and capture cpu/mem/time.
    Returns
    -------
    model : fitted LocalOutlierFactor
    fit_usage : dict with time_sec, rss_before/after, cpu_before/after, and params
    """
...
def predict(model, X, df=None, group_col="stNum"):
    raw_scores = model.decision_function(X)   # typical sign: higher = more normal

    # convert to anomaly-oriented score: higher => more anomalous
    scores = -raw_scores
    labels = model.predict(X)                         # -1 anomalous, +1 normal
    y_pred = np.where(labels == -1, 1, 0).astype(int)

    return y_pred, scores


# ==============================================================================
# Main (uniform signature & outputs like IQR)
# ==============================================================================
def main(train_input_path=None,
    ...
    # Put features back for any downstream exports
    df_train[cols] = X_train; df_val[cols] = X_val; df_test[cols] = X_test

    # Infer hierarchy like IQR and prep dirs
    ...
    # Grouping column parity (like IQR)
    group_col = "stNum"
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler:
                ...
                for _ in range(N_LOOPS):
                    ...
                    model, X_test, df=df_test, group_col=group_col
                )
