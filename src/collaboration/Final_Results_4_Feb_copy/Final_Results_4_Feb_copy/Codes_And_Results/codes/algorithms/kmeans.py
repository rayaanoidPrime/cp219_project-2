...
#   Core model
# =========================
def run_kmeans(X_train, random_state=None):
    """
    Fit single-cluster KMeans on TRAIN and compute threshold from TRAIN distances.
    """
    model = KMeans(n_clusters=1, random_state=random_state, n_init='auto', init='k-means++',)
    model.fit(X_train)

    d_train = model.transform(X_train)
    min_d_train = np.min(d_train, axis=1)
    threshold = float(np.percentile(min_d_train, 95))
...
def predict(X, model: KMeans, threshold: float):
    """Threshold nearest-centroid distance to decide anomalies."""

    d = model.transform(X)
    min_d = np.min(d, axis=1)
    y_pred = (min_d > threshold).astype(int)
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):
                    model, threshold, config = run_kmeans(X_train, random_state=i * 10)
