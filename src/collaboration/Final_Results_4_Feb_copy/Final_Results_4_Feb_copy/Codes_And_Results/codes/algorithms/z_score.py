...
#   Core helpers
# =========================
def run_z_score(X_train, y_train, X_val, y_val, random_state=None):
    """
    Fit normal stats on train (means/stds). Do a quick validation pass to
    record resource use. Returns (means, stds, config).
    """
...
def predict(X, means, stds, eps=1e-8, clip_max=1e6, threshold=3.0):
    z = np.abs(X - means) / (stds + eps)
    z = np.clip(z, 0, clip_max)

    scores = np.max(z, axis=1)        # anomaly score
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):        
                    means, stds =  run_z_score(X_train, y_train, X_val, y_val, random_state=i)
            ...
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):        
                    y_test_pred, scores  = predict(X_test, means, stds)
