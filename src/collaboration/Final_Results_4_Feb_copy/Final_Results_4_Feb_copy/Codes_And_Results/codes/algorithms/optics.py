...
def run_optics(X_test, min_samples=10, xi=0.05, percentile=95):
    """
    Final benchmark implementation for OPTICS.
    Note: Fitted on X_test because OPTICS is transductive.
    """
    # 1. Initialize and Fit
    # min_samples is the core hyperparameter for density
    model = OPTICS(min_samples=min_samples, xi=xi, metric='euclidean')
    model.fit(X_test)

    # 2. Extract Reachability Distances
    # Reachability is the 'Anomaly Score' for OPTICS
    reach = model.reachability_.copy()

    # 3. Handle Infinite Distances
    # OPTICS produces 'inf' for points that are extremely isolated.
    # We must replace them with a finite value for PR-AUC/ROC-AUC to work.
    finite_mask = np.isfinite(reach)
    if np.any(finite_mask):
        max_reach = np.max(reach[finite_mask])
        # Assign 10% higher than max to preserve 'most anomalous' ranking
        scores = np.where(finite_mask, reach, max_reach * 1.1)
    else:
        scores = np.zeros_like(reach)

    # 4. Thresholding
    # We use the percentile (tuned on X_val) to create binary labels
...
def main(train_input_path=None,
    ...
    # ---------- Extract features/labels like IQR ----------
    ...
    for i in range(1, num_runs + 1):
        run_key = f"Run_{i}"
        try:
            # ---------------- Fit OCSVM (capture resources) ----------------
            with ru.ResourceProfiler() as profiler_train:
                # Loop the training run_optics(X_test, min_samples=10)ask N_LOOPS times
                for _ in range(N_LOOPS):        
                    _, _ = run_optics(X_test)
            ...
            with ru.ResourceProfiler() as profiler:
                ...
                for _ in range(N_LOOPS):
                    y_test_pred, scores = run_optics(X_test)
            ...
            if np.sum(finite_mask) < 2 or len(np.unique(y_test[finite_mask])) < 2:
                pr_auc = 1e-8
                roc_auc = 1e-8
            else:
                pr_auc = average_precision_score(y_test[finite_mask], scores[finite_mask])
                roc_auc = roc_auc_score(y_test[finite_mask], scores[finite_mask])
