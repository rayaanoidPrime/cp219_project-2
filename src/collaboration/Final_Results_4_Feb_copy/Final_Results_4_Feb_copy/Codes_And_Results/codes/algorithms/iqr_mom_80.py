...
def run_window_statistical(
    ...
    """
    Train statistical detector using fixed window size.
    """
    ...
    # --- Compute window mean scores ---
    window_means = []

    for start in range(0, len(row_scores), window_size):
        end = min(start + window_size, len(row_scores))
        window_means.append(np.mean(row_scores[start:end]))

    window_means = np.array(window_means)

    # --- Compute threshold ---
    median_score = np.median(window_means)
    q75, q25 = np.percentile(window_means, [75, 25])
    iqr_score = q75 - q25

    upper_threshold = float(median_score + 1.5 * iqr_score)
...
def predict_window_fast(
    ...
    """
    Fast prediction using fixed window size.
    """
    ...
    for start in range(0, len(X), window_size):
        ...
        window_scores = row_scores_all[start:end]

        if np.mean(window_scores) > upper_threshold:

            # Only mark rows that have strong feature deviation
            window_feat_scores = feature_scores[start:end]
            mask = np.any(window_feat_scores >= 3, axis=1)

            y_pred[start:end][mask] = 1
