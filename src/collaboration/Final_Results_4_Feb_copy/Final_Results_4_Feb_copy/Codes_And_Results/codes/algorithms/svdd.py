...
def get_center(model, train_loader, device):
    """Initializes the center c as the mean of the network outputs."""
    model.eval()
    all_z = []
    with torch.no_grad():
        for data in train_loader:
            ...
            z = model(x)
            all_z.append(z.cpu())
    center = torch.cat(all_z).mean(dim=0)
    # Ensure center is not zero to avoid trivial solutions
    center[center.abs() < 0.1] = 0.1
    return center.to(device)


def fit_svdd(X, random_state=42, latent_dim=16, epochs=50, lr=1e-3, batch_size=64):
    torch.manual_seed(random_state)
    device = torch.device("cpu")
    
    X_tensor = torch.tensor(X, dtype=torch.float32)
    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(X_tensor), batch_size=batch_size, shuffle=True
    )

    model = SVDDNet(input_dim=X.shape[1], latent_dim=latent_dim).to(device)
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=1e-6)

    # 1. Initialize Center c
    center = get_center(model, train_loader, device)
    ...
    for epoch in range(epochs):
        for data in train_loader:
            ...
            optimizer.zero_grad()
            z = model(x)
            # Loss = Average Squared Euclidean Distance to Center
            loss = torch.mean(torch.sum((z - center)**2, dim=1))
            loss.backward()
            optimizer.step()

    model.center = center # Store center in the model for prediction
    model.device = device
    return model


def predict(X_test, model, percentile=95):
    model.eval()
    device = model.device
    ...
    with torch.no_grad():
        ...
        distances = torch.sum((z - model.center)**2, dim=1).cpu().numpy()
    
    # Continuous scores for PR-AUC
    scores = distances
...
def main(train_input_path=None,
    ...
    for i in range(1, num_runs + 1):
        run_key = f"Run_{i}"
        try:
            # ---------- Train on TRAIN (1 cluster) ----------
            with ru.ResourceProfiler() as profiler_train:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    model = fit_svdd(X_train, random_state=i * 10)
            
            # Get the *average* time for one run in nanoseconds
            ...
            # Replace the 25+ lines of profiling with this block
            with ru.ResourceProfiler() as profiler:
                for _ in range(N_LOOPS):
                        y_test_pred, scores = predict(X_test, model)
