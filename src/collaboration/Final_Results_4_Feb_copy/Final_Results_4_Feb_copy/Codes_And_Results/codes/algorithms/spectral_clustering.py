...
def run_sc(
    ...
    """
    Zero-shot transductive Spectral Clustering for anomaly detection.
    """

    # 1. Build affinity (RBF kernel)
    A = rbf_kernel(X, gamma=gamma)

    # 2. Spectral embedding (TRANSDUCTIVE)
    Z = spectral_embedding(
        A,
    ...
    # 4. Spectral cluster centers
    centers = np.vstack([
        Z[cluster_ids == c].mean(axis=0)
        for c in range(n_clusters)
    ])

    # 5. Anomaly scores: distance to nearest spectral center
    dists = pairwise_distances(Z, centers)
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):
                    _,_ = run_sc(X_test, random_state=i * 10)
            ...
            with ru.ResourceProfiler() as profiler:
                # Loop the training task N_LOOPS times
                for _ in range(N_LOOPS):
                    y_test_pred, scores= run_sc(X_test, random_state=i * 10)
