# =============================================================
# AUTOENCODER (AE) for Unsupervised Anomaly Detection
# =============================================================

import os
# Completely hide all GPUs from TensorFlow
os.environ["CUDA_VISIBLE_DEVICES"] = ""

# Disable XLA GPU device creation
...
def setup_tf_device():
    """
    Force TensorFlow to run on CPU only, even if GPUs are available.
    We want to standardise the runs across all algorithms
    """
...
# =============================================================
def run_ae(X_tr, y_tr, random_state):
    """
    Autoencoder trained ONLY on normal data (X_tr).
    Different random_state => different weight initialization.
    No validation data used.
    """

    # =====================================================
    # 1. SET RANDOM SEEDS (ONCE PER RUN)
    # =====================================================
    ...
    encoding_dim_1 = int(input_dim * 0.5)
    encoding_dim_2 = int(input_dim * 0.25)
    bottleneck_dim = max(1, int(input_dim * 0.125))
    ...
    with tf.device(DEVICE):
        ...
        enc2 = Dense(encoding_dim_2, activation="relu")(enc1)
        bottleneck = Dense(bottleneck_dim, activation="relu")(enc2)

        dec1 = Dense(encoding_dim_2, activation="relu")(bottleneck)
        dec2 = Dense(encoding_dim_1, activation="relu")(dec1)
        out = Dense(input_dim, activation=None)(dec2)
    ...
    # =====================================================
    # 2. TRAINING RECONSTRUCTION ERROR
    # =====================================================
    train_recon = autoencoder.predict(X_tr, verbose=0)
    train_mse = np.mean((train_recon - X_tr) ** 2, axis=1)

    # Same protocol as other baselines
...
def main(train_input_path=None,
    ...
    df_val   = pd.read_csv(validation_input_path) if validation_input_path and os.path.exists(validation_input_path) else df_test.copy()
        ...
        try:
            ...
            with ru.ResourceProfiler() as profiler_train:
                ...
                for _ in range(N_LOOPS):
                    model, threshold = run_ae(X_train, y_train, random_state=i * 10)
